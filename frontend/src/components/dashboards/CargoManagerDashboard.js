              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {bookings.map((booking) => (
                    <tr 
                      key={booking._id}
                      className={(booking.status === 'Accepted' || booking.status === 'Declined') ? 'cursor-pointer hover:bg-gray-50' : ''}
                      onClick={() => (booking.status === 'Accepted' || booking.status === 'Declined') && handleShowBookingDetails(booking)}
                    >
